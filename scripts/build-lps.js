...
/**
 * Resolve the small-screen variant of a hero image.
 *
 * The hero is a CSS background on .lp-hero::before, and the same file used to be
 * served to a 412px phone and a 1920px desktop alike — the heaviest was 553KB.
 * Where a `{name}-sm.webp` exists next to the full-size file we serve it below
 * 768px; otherwise we fall back to the full-size image, so LPs whose hero still
 * lives in images/practice-areas/ (already 800x800 and small) need no variant.
 *
 * Paths in configs are page-relative ("../images/lp/x.webp"), so resolve against
 * the site root to test for the file on disk.
 */
function smallHeroFor(heroImage) {
    if (!heroImage || !heroImage.endsWith('.webp')) return heroImage;
    const candidate = heroImage.replace(/\.webp$/, '-sm.webp');
    const onDisk = path.join(ROOT, candidate.replace(/^\.\.\//, ''));
    return fs.existsSync(onDisk) ? candidate : heroImage;
}
...
function buildOne(template, config) {
    ...
    const replacements = {
        ...
        HERO_IMAGE_SM:       smallHeroFor(config.heroImage),
        H1_PRE:              config.h1Pre,
...
// Read once at module scope so buildOne() can inline it. Generated by
// scripts/build-lp-critical-css.js, which `npm run build` runs first.
const criticalCss = (() => {
    if (!fs.existsSync(CRITICAL_CSS)) {
        console.error(`Missing ${path.relative(ROOT, CRITICAL_CSS)} — run: npm run build:lp-css`);
        process.exit(1);
    }
    return fs.readFileSync(CRITICAL_CSS, 'utf8').trim();
})();
